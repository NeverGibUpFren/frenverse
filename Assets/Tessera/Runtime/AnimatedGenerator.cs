...
namespace Tessera
{
  ...
  public class AnimatedGenerator : MonoBehaviour
  {
    ...
    private void OnInitialized()
    {
      ...
      tileOutput = generator.GetTileOutputs(true)[0];
    ...
    public void StopGeneration()
    {
      ...
      initCts?.Cancel();
      tileOutput = null;
      if (helper != null && cubesByIndex != null)
      {
    ...
    private GameObject CreateCube(TesseraGenerator generator, Sylves.IGrid grid, Sylves.Cell p)
    {
      ...
      var trs = Sylves.TRS.World(generator.transform) * grid.GetTRS(p);
    ...
    public void Step()
    {
      ...
      foreach (var i in changeTracker.GetChangedIndices())
      {
        ...
        else if (hasChanged)
        {
          ...
          if (uncertaintyTile != null && supportsCubes)
          {
            ...
            var uncertaintyInspector = c.GetComponent<TesseraUncertainty>();
            if (uncertaintyInspector != null)
            {
              uncertaintyInspector.modelTiles = after;
            }
          }
          ...
          if (hasObject[i])
          {
            updateTiles[(Vector3Int)p] = new ModelTile();
            hasObject[i] = false;
          }
        }
        lastTiles[i] = after;
      ...
      if (propagator.Status != DeBroglie.Resolution.Undecided)
      {
        if (propagator.Status == DeBroglie.Resolution.Contradiction)
        {
          ...
          var contradictionLocation = SylvesDeBroglieUtils.GetContradictionLocation(result, grid);
